...
export const Form = ({
    ...
    return (
        <div className='bg-white w-[800px] h-[800px] shadow-lg rounded-lg flex flex-col justify-center items-center'>
            <div className='text-4xl font-extrabold'>Welcome {isSignInPage ? 'back' : ''}</div>
            <div className='text-xl font-light'>
                <div className='mb-10'> <center>{isSignInPage ? 'Sign in to explore' : 'Sign up for Now to get started'}</center></div>
                <form onSubmit={(e) => {
                        e.preventDefault();
                    ...
                    <div className='text-sm'>{isSignInPage ? "Didn't have an account?" : "Already have an account? "}<span className=' text-primary cursor-pointer underline mb-6'>{isSignInPage ? 'Sign In' : 'Sign Up'}</span></div>
                </form>
            </div>
